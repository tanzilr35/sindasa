    ...
    <div class="container">
        <div class="row align-items-center">
            ...
            <div class="col-lg-6 text-lg-right">
                ...
                <a class="btn btn-dark btn-social mx-2" href="https://dinkes.madiunkab.go.id/"><i class="fas fa-envelope"></i></a>
            </div>
        </div>
    </div>
</footer>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#nik').autocomplete({
            source: "<?php echo site_url('Anak/get_autocomplete'); ?>",

            select: function(event, ui) {
                console.log(ui.item)
                $('[name="nik"]').val(ui.item.label);
                $('[name="nama"]').val(ui.item.nama);
                $('[name="nama_ibu"]').val(ui.item.nama_ibu);
            },
